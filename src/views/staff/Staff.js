  ...
  useEffect(()=>{
    // console.log("inside useffect staff",staffId);
       const getStaff = async()=>{
           try{
               const response = await fincoDefault.get(`/finco/api/auth/staff/list?officeId=${staffId}`,{
                                   headers:{
                                       "token":`${token}`
                                   }
                                   }) 
                ...
                <CCol xs="12">
                    <CCard style={{background:"#60779f"}}>
                        <CCardHeader component="h5" >
                            ...
                            <CIcon icon={cilUser} customClassName="nav-icon" height={30} width={50} style={{color:"white"}}/>
